...
//						echo "You will be automatically redirected after 5 seconds.";
					...
					elseif (isset($_GET['e'])) echo "<div class=\"alert alert-error\">"
					.$_GET['e']."</div>";
			?> 
			<form name="contactForm" action="<?php echo site_url() ?>/TourController/registerseat/<?php echo $this->uri->segment(3) ?>" method="POST" class="form-horizontal">
				<div class='control-group'>
					...
					<?php 
						for($i=1; $i<31; $i++)
						{
							...
							if(isset($_POST[$chparam]))
							{
								...
								echo "<input type='text' class='span3' name=ch".$i." value=".$i." readonly style='float:left;width:30px;margin-left:10px;text-align:center' />";
					...
					<?php
						...
						if(isset($_POST['doj']))
						{
							...
							echo "<label class='control-label' for='input1'>Tour Date</label>";
	            ...
	            <div class="control-group">
	                ...
	                <div class="controls">
	                    <input type="text" class="span3" placeholder="Type your email id" name="email" pattern="^[a-zA-Z0-9-\_.]+@[a-zA-Z0-9-\_.]+\.[a-zA-Z0-9.]{2,5}$" title="Please enter a valid email id." required readonly value="<?php echo $this->session->userdata('useremail') ?>"/>
	                </div>
	            </div>
